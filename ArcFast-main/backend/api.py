...
Contrato de API idéntico al original — el frontend no cambia nada.
Fixes internos respecto al original:
  - /grabar/terminar es async def → usa await analizar_sesion() del brain
  - /ejecutar es async def → elimina asyncio.run() que deadlockeaba Uvicorn
  - completar_plan devuelve solo el plan (no tupla) — compatible con el frontend
  - Credenciales no se persisten en DB
"""
